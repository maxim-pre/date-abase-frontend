    ...
    return (
        <div className="border w-100 mx-4 my-4 px-4 py-4 bg-[#e8e8e890]">
            ...
            <div className='flex flex-col items-center'>
                <div>
                {isMatched && <button className="text-4xl bg-red-500 rounded-full px-5 py-4 mr-6" onClick={handleRemoveMatch}>N</button>}
                {!isMatched && <button className="text-4xl bg-green-500 rounded-full px-6 py-4 ml-6" onClick={handleAddMatch}> Y</button>}
                </div>
                </div>
        </div>
    )
